...
import com.example.yutee.weatherforecast.data.db.LocalDateConverter
import com.example.yutee.weatherforecast.internal.DateNotFoundException
...
class FutureDetailWeatherFragment : ScopedFragment(), KodeinAware {
    ...
    private fun bindUI() = launch(Dispatchers.Main) {
        ...
        futureWeather.observe(this@FutureDetailWeatherFragment, Observer { weatherEntry ->
            ...
            GlideApp.with(this@FutureDetailWeatherFragment)
                ...
                .into(imageView_condition_icon_fd)
        })
    }
    ...
    private fun updateTemperatures(temperature: Double, min: Double, max: Double) {
        ...
        textView_temperature_fd.text = "$temperature$unitAbbreviation"
        textView_min_max_temperature_fd.text = "Min: $min$unitAbbreviation, Max: $max$unitAbbreviation"
    }

    private fun updateCondition(condition: String) {
        textView_condition_fd.text = condition
    }

    private fun updatePrecipitation(precipitationVolume: Double) {
        val unitAbbreviation = chooseLocalizedUnitAbbreviation("mm", "in")
        textView_precipitation_fd.text = "Precipitation: $precipitationVolume $unitAbbreviation"
    }

    private fun updateWindSpeed(windSpeed: Double) {
        val unitAbbreviation = chooseLocalizedUnitAbbreviation("kph", "mph")
        textView_wind_fd.text = "Wind speed: $windSpeed $unitAbbreviation"
    }

    private fun updateVisibility(visibilityDistance: Double) {
        val unitAbbreviation = chooseLocalizedUnitAbbreviation("km", "mi.")
        textView_visibility_fd.text = "Visibility: $visibilityDistance $unitAbbreviation"
    }

    private fun updateUv(uv: Double) {
        textView_uv_fd.text = "UV: $uv"
    }
}
